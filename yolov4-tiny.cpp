...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov4-tiny.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    auto pool9 = network->addPooling(*cat8->getOutput(0), PoolingType::kMAX, DimsHW{2, 2});
    pool9->setStride(DimsHW{2, 2});
    ...
    auto pool17 = network->addPooling(*cat16->getOutput(0), PoolingType::kMAX, DimsHW{2, 2});
    pool17->setStride(DimsHW{2, 2});
    ...
    auto pool25 = network->addPooling(*cat24->getOutput(0), PoolingType::kMAX, DimsHW{2, 2});
    pool25->setStride(DimsHW{2, 2});
    ...
    IConvolutionLayer* conv29 = network->addConvolution(*l28->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.29.Conv2d.weight"], weightMap["module_list.29.Conv2d.bias"]);
    assert(conv29);
    // 30 is a yolo layer
    ...
    IDeconvolutionLayer* deconv33 = network->addDeconvolution(*l32->getOutput(0), 128, DimsHW{2, 2}, deconvwts33, emptywts);
    assert(deconv33);
    deconv33->setStride(DimsHW{2, 2});
    ...
    IConvolutionLayer* conv36 = network->addConvolution(*l35->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.36.Conv2d.weight"], weightMap["module_list.36.Conv2d.bias"]);
    assert(conv36);
    // 37 is a yolo layer

    auto yolo = new YoloLayerPlugin();
    ITensor* inputTensors_yolo[] = {conv29->getOutput(0), conv36->getOutput(0)};
    auto yolo_ = network->addPlugin(inputTensors_yolo, 2, *yolo);
    assert(yolo_);
    yolo_->setName("yolo_");
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov4-tiny.engine");
        if (!p) {
